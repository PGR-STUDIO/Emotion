...
function report() {
  ...
  const observationCards = state.entries.slice().reverse().map((entry, index) => `<article class="observation"><div class="observation-head"><div><p class="eyebrow">OBSERVATION ${state.entries.length - index}</p><h2>${text(labels[entry.emotion] || entry.emotion)}</h2></div><strong>${entry.intensity}/10</strong></div><p class="date">${text(new Date(entry.date).toLocaleString('fr-FR'))}</p><div class="grid"><section><h3>Situation</h3><p>${text(entry.situation)}</p></section><section><h3>Pensées</h3><p>${text(entry.thoughts)}</p></section><section><h3>Signes et ressentis</h3><p>${text((entry.bodySigns || []).join(', '))}</p></section><section><h3>Besoin possible</h3><p>${text(entry.need)}</p></section><section><h3>Réaction</h3><p>${text(entry.reaction)}</p></section><section><h3>Après</h3><p>${text(entry.consequence)}</p></section></div>${entry.nuance ? `<p class="tagline"><b>Nuance :</b> ${text(entry.nuance)}</p>` : ''}${entry.exercise ? `<div class="exercise"><h3>Exercice</h3><p>${text(entry.exercise)}${entry.exerciseDuration ? ` · ${entry.exerciseDuration} secondes` : ''}</p><p><b>Ressenti :</b> ${resultLabel(entry.exerciseResult)}${entry.exerciseAfterIntensity == null ? '' : ` · intensité après : ${entry.exerciseAfterIntensity}/10`}</p></div>` : ''}${entry.question ? `<p class="question"><b>Question pour la séance :</b> ${text(entry.question)}</p>` : ''}</article>`).join('');
  const html = `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Résumé de séance — Émotions</title><style>:root{color-scheme:light}*{box-sizing:border-box}body{margin:0;background:#f5f6f1;color:#29435f;font:16px/1.55 -apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}.page{max-width:860px;margin:0 auto;padding:48px 24px}.cover,.observation{background:#fffefa;border:1px solid #dce8df;border-radius:24px;box-shadow:0 10px 30px #29435f12}.cover{padding:38px;margin-bottom:22px}.brand{color:#85b79b;font-size:13px;font-weight:800;letter-spacing:.18em}.cover h1{margin:8px 0 10px;font-size:38px;line-height:1.1}.cover p{color:#627586;margin:0}.summary{display:inline-block;margin-top:24px;padding:9px 14px;border-radius:999px;background:#e7f2e7;color:#38734b;font-weight:700}.observation{padding:28px;margin:18px 0;break-inside:avoid}.observation-head{display:flex;align-items:flex-start;justify-content:space-between;gap:16px;border-bottom:1px solid #e3ece4;padding-bottom:14px}.eyebrow{margin:0;color:#85b79b;font-size:12px;font-weight:800;letter-spacing:.16em}.observation h2{margin:4px 0 0;font-size:28px}.observation-head strong{color:#85b79b;font-size:27px}.date{color:#7a8994;font-size:13px;margin:12px 0 18px}.grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:12px}.grid section{background:#f7faf5;border-radius:14px;padding:12px 14px;min-height:76px}.grid h3,.exercise h3{font-size:13px;margin:0 0 4px;color:#6d987d}.grid p,.exercise p{margin:0}.tagline,.question{padding:12px 14px;border-left:4px solid #9ac4a7;background:#f7faf5}.exercise{margin-top:16px;padding:16px;border-radius:16px;background:#eaf3ea}.question{margin-bottom:0}@media(max-width:600px){.page{padding:24px 14px}.cover{padding:26px 20px}.cover h1{font-size:31px}.grid{grid-template-columns:1fr}}@media print{body{background:#fff}.page{padding:0}.cover,.observation{box-shadow:none}}</style></head><body><main class="page"><header class="cover"><div class="brand">ÉMOTIONS</div><h1>Résumé de séance</h1><p>Une trace claire de ce que tu as observé, ressenti et essayé.</p><div class="summary">${state.entries.length} observation${state.entries.length > 1 ? 's' : ''}</div></header>${observationCards || '<p>Aucune observation enregistrée.</p>'}</main></body></html>`;
