...
function freshDraft() { return { emotion: null, nuance: '', intensity: 5, bodySigns: [], exercise: '', exerciseDuration: null, exerciseStatus: '', exerciseAfterIntensity: null, exerciseResult: '' }; }
...
function renderEmotions() {
  ...
  document.querySelectorAll('[data-emotion]').forEach(button => button.onclick = () => {
    ...
    draft.exercise = ''; draft.exerciseStatus = ''; draft.exerciseAfterIntensity = null; draft.nuance = ''; draft.bodySigns = [];
...
function syncContinueButton() { const button = $('#toUnderstand'); const hasChoice = Boolean(draft.emotion || draft.nuance); button.disabled = !hasChoice; button.firstChild.textContent = hasChoice ? 'Comprendre cette émotion ' : 'Choisis une émotion '; }
...
function renderWheel() {
  ...
  const primaryRadius = 50;
  const declinationRing = [56, 116];
  const blendRing = [122, 176];
  const visibleBlends = wheelBlends.filter(blend => blend.canonical);
  const primarySvg = wheelData.map((family, familyIndex) => { const start = familyIndex * 45 - 21.7; const end = start + 43.4; return `<path class="interactive-sector primary-sector ${draft.nuance === family.levels[1] ? 'selected' : ''}" data-nuance="${escapeHtml(family.levels[1])}" role="button" tabindex="0" aria-label="Choisir ${escapeHtml(wheelItems[familyIndex])}" d="${primarySector(primaryRadius, start, end)}" fill="${family.colors[1]}"><title>${escapeHtml(wheelItems[familyIndex])}</title></path>`; }).join('');
  const primaryLabels = { 'Confiance': 'Conf.', 'Surprise': 'Surpr.', 'Tristesse': 'Trist.', 'Anticipation': 'Antic.' };
  const primaryLabelsSvg = wheelItems.map((label, familyIndex) => { const angle = familyIndex * 45; const [x, y] = point(29, angle); const readableAngle = angle > 90 && angle < 270 ? angle + 180 : angle; return `<text class="wheel-primary-label" x="${x}" y="${y}" transform="rotate(${readableAngle} ${x} ${y})">${escapeHtml(primaryLabels[label] || label)}</text>`; }).join('');
  const declinationSvg = wheelData.flatMap((family, familyIndex) => [2, 0].map((levelIndex, declinationIndex) => {
    const familyStart = familyIndex * 45 - 21.7;
    const half = 21.7;
    const start = familyStart + declinationIndex * half + 0.35;
    const end = familyStart + (declinationIndex + 1) * half - 0.35;
    const word = family.levels[levelIndex];
    const displayLabel = wheelDisplayLabel(word);
    return `<path class="interactive-sector declination-sector ${draft.nuance === word ? 'selected' : ''}" data-nuance="${escapeHtml(word)}" role="button" tabindex="0" aria-label="Choisir ${escapeHtml(displayLabel)}" aria-pressed="${draft.nuance === word}" d="${sector(declinationRing[0], declinationRing[1], start, end)}" fill="${family.colors[levelIndex]}"><title>${escapeHtml(displayLabel)}</title></path>`;
  })).join('');
  const blendSvg = visibleBlends.map((blend, blendIndex) => { const step = 360 / visibleBlends.length; const start = blendIndex * step - step / 2; const end = start + step - 0.35; return `<path class="interactive-sector blend-sector ${draft.nuance === blend.name ? 'selected' : ''}" data-nuance="${escapeHtml(blend.name)}" role="button" tabindex="0" aria-label="Choisir ${escapeHtml(blend.name)}" d="${sector(blendRing[0], blendRing[1], start, end)}" fill="${wheelData[blend.firstIndex].colors[1]}"><title>${escapeHtml(blend.name)} — ${escapeHtml(blend.pair)}</title></path>`; }).join('');
  let selectedOverlay = '';
  wheelData.forEach((family, familyIndex) => {
    if (draft.nuance === family.levels[1]) {
      const start = familyIndex * 45 - 21.7; const end = start + 43.4; const selectedShape = primarySector(primaryRadius, start, end);
      selectedOverlay = `<path class="wheel-selection-halo" d="${selectedShape}"></path><path class="wheel-selection-overlay" d="${selectedShape}" stroke="${family.colors[1]}"></path>`;
    }
    [2, 0].forEach((levelIndex, declinationIndex) => {
      if (draft.nuance !== family.levels[levelIndex]) return;
      const familyStart = familyIndex * 45 - 21.7; const start = familyStart + declinationIndex * 21.7 + 0.35; const end = familyStart + (declinationIndex + 1) * 21.7 - 0.35; const selectedShape = sector(declinationRing[0], declinationRing[1], start, end);
      selectedOverlay = `<path class="wheel-selection-halo" d="${selectedShape}"></path><path class="wheel-selection-overlay" d="${selectedShape}" stroke="${family.colors[levelIndex]}"></path>`;
    });
  });
  visibleBlends.forEach((blend, blendIndex) => { if (draft.nuance !== blend.name) return; const step = 360 / visibleBlends.length; const start = blendIndex * step - step / 2; const end = start + step - 0.35; const selectedShape = sector(blendRing[0], blendRing[1], start, end); selectedOverlay = `<path class="wheel-selection-halo" d="${selectedShape}"></path><path class="wheel-selection-overlay" d="${selectedShape}" stroke="${wheelData[blend.firstIndex].colors[1]}"></path>`; });
  const declinationLabelPaths = wheelData.flatMap((family, familyIndex) => [2, 0].map((levelIndex, declinationIndex) => { const familyStart = familyIndex * 45 - 17; const start = familyStart + declinationIndex * 17; const end = start + 15; const [x1, y1] = point(86, start); const [x2, y2] = point(86, end); return `<path id="wheel-declination-label-${familyIndex}-${declinationIndex}" class="wheel-label-path" d="M ${x1} ${y1} A 86 86 0 0 1 ${x2} ${y2}"/>`; })).join('');
  const blendLabelPaths = visibleBlends.map((blend, blendIndex) => { const step = 360 / visibleBlends.length; const start = blendIndex * step - 17; const end = start + 34; const forward = blendIndex < 4; const [x1, y1] = point(148, forward ? start : end); const [x2, y2] = point(148, forward ? end : start); return `<path id="wheel-blend-label-${blendIndex}" class="wheel-label-path" d="M ${x1} ${y1} A 148 148 0 0 ${forward ? 1 : 0} ${x2} ${y2}"/>`; }).join('');
  const labelsInWheel = wheelData.flatMap((family, familyIndex) => [2, 0].map((levelIndex, declinationIndex) => `<text class="wheel-arc-label wheel-declination-label"><textPath href="#wheel-declination-label-${familyIndex}-${declinationIndex}" startOffset="50%">${wheelDisplayLabel(family.levels[levelIndex])}</textPath></text>`)).join('') + visibleBlends.map((blend, blendIndex) => `<text class="wheel-arc-label wheel-blend-label"><textPath href="#wheel-blend-label-${blendIndex}" startOffset="50%">${escapeHtml(blend.name)}</textPath></text>`).join('');
  const legend = wheelData.map((family, index) => `<span><i style="background:${family.colors[1]}"></i>${wheelItems[index]}</span>`).join('');
  $('#wheelPanel').innerHTML = `<p>Centre : 8 émotions de base · deuxième roue : 16 déclinaisons · troisième roue : 8 mélanges.</p><div class="interactive-wheel-wrap"><svg class="interactive-wheel" viewBox="0 0 360 360" role="group" aria-label="Roue interactive des émotions de Plutchik"><defs>${declinationLabelPaths}${blendLabelPaths}</defs>${blendSvg}${declinationSvg}${primarySvg}${primaryLabelsSvg}${selectedOverlay}<circle cx="180" cy="180" r="18" class="wheel-center"></circle>${labelsInWheel}</svg></div><div class="wheel-legend">${legend}</div><div class="wheel-extra-emotions"><b>Autres émotions disponibles · 17 nuances</b><div>${extraEmotionOptions.map(word => `<button type="button" class="extra-emotion ${draft.nuance === word ? 'selected' : ''}" data-nuance="${escapeHtml(word)}" aria-pressed="${draft.nuance === word}"><span>${escapeHtml(word)}</span></button>`).join('')}</div></div><details class="wheel-blends"><summary>8 mélanges principaux · 20 chevauchements possibles</summary><div>${wheelBlends.map(blend => `<span style="display:flex;flex-direction:column;align-items:flex-start;gap:2px;min-width:0"><b>${escapeHtml(blend.name)}</b><small>${blend.canonical ? 'Mélange principal' : 'Chevauchement possible'} · ${escapeHtml(blend.pair)}</small></span>`).join('')}</div></details><div class="wheel-selection">${draft.nuance ? `Émotion choisie : ${escapeHtml(wheelDisplayLabel(draft.nuance))}` : 'Aucune émotion choisie pour le moment.'}</div><p class="wheel-credit">Roue inspirée du modèle de Robert Plutchik : les 8 familles sont au centre, leurs deux pôles d’intensité sur la deuxième roue, et les 8 mélanges principaux sur la troisième.</p>`;
}
...
function report() {
  ...
  const observationCards = state.entries.slice().reverse().map((entry, index) => `<article class="observation"><div class="observation-head"><div><p class="eyebrow">OBSERVATION ${state.entries.length - index}</p><h2>${text(labels[entry.emotion] || entry.emotion)}</h2></div><strong>${entry.intensity}/10</strong></div><p class="date">${text(new Date(entry.date).toLocaleString('fr-FR'))}</p><div class="grid"><section><h3>Nuance</h3><p>${text(entry.nuance)}</p></section><section><h3>Situation</h3><p>${text(entry.situation)}</p></section><section><h3>Pensées</h3><p>${text(entry.thoughts)}</p></section><section><h3>Signes et ressentis</h3><p>${text((entry.bodySigns || []).join(', '))}</p></section><section><h3>Exercice essayé</h3><p>${text(entry.exercise)}</p></section><section><h3>Durée prévue</h3><p>${entry.exerciseDuration ? `${entry.exerciseDuration} secondes` : 'Non renseignée'}</p></section><section><h3>Statut de l’exercice</h3><p>${text(entry.exerciseStatus)}</p></section><section><h3>Intensité après l’exercice</h3><p>${entry.exerciseAfterIntensity == null ? 'Non renseignée' : `${entry.exerciseAfterIntensity}/10`}</p></section><section><h3>Ressenti après exercice</h3><p>${resultLabel(entry.exerciseResult)}</p></section><section><h3>Besoin possible</h3><p>${text(entry.need)}</p></section><section><h3>Réaction</h3><p>${text(entry.reaction)}</p></section><section><h3>Après</h3><p>${text(entry.consequence)}</p></section></div><p class="question"><b>Question pour la séance :</b> ${text(entry.question === 'Aucune' ? '' : entry.question)}</p></article>`).join('');
...
$('#unknownButton').onclick = () => { draft.emotion = 'unknown'; draft.nuance = ''; draft.exercise = ''; saveDraft(); renderEmotions(); renderBodySigns(); renderEmotionInsight(); renderExercise(); syncContinueButton(); };
$('#wheelToggle').onclick = () => { const open = !$('#wheelPanel').classList.contains('open'); $('#wheelPanel').classList.toggle('open', open); $('#observe').classList.toggle('wheel-mode', open); $('#wheelToggle').textContent = open ? 'Revenir aux émotions principales' : 'Préciser avec la roue des émotions'; draft.emotion = null; draft.nuance = ''; draft.exercise = ''; draft.exerciseStatus = ''; draft.exerciseAfterIntensity = null; saveDraft(); renderEmotions(); renderEmotionInsight(); renderExercise(); renderWheel(); syncContinueButton(); };
