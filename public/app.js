...
const PRIVACY_TEST_MODE = QUERY.get('privacy-test') === '1';
const STORAGE_SUFFIX = PRIVACY_TEST_MODE ? '-test' : '';
const KEY = `pgr-simple-final${STORAGE_SUFFIX}`;
const DRAFT_KEY = `pgr-simple-final-draft${STORAGE_SUFFIX}`;
const SECURE_KEY = `pgr-simple-final-secure${STORAGE_SUFFIX}`;
const SECURE_DRAFT_KEY = `pgr-simple-final-draft-secure${STORAGE_SUFFIX}`;
const BROWSER_TEST_MODE = QUERY.get('browser-test') === '1';
const LOCAL_ONLY_MODE = !PRIVACY_TEST_MODE && !BROWSER_TEST_MODE;
...
const LOCAL_JSON_PATH = /^scientific_knowledge_base\/(?:data\/[a-z0-9_-]+\.json|source_config\.json)$/;
function loadJson(path) {
  if (!LOCAL_JSON_PATH.test(path)) return Promise.reject(new Error(`Chemin de données local refusé : ${path}`));
  return fetch(`./${path}`).then(response => { if (!response.ok) throw new Error(`${response.status} ${path}`); return response.json(); });
}
...
function normalizeText(value = '') { return String(value).normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[’‘ʼ]/g, "'").replace(/\u00a0/g, ' ').toLowerCase(); }
...
function evidenceLabel(grade) { return scientific.evidence?.grades?.[grade] ? `Repère ${grade} · ${scientific.evidence.grades[grade]}` : `Repère ${grade || '—'}`; }
...
const wheelDisplayLabel = word => word === 'Colère' ? 'En colère' : word;
...
let state = { entries: [] };
let legacySecureStorageDetected = LOCAL_ONLY_MODE && Boolean(localStorage.getItem(SECURE_KEY));
let secureMode = !LOCAL_ONLY_MODE && Boolean(localStorage.getItem(SECURE_KEY));
let stateLocked = !LOCAL_ONLY_MODE && secureMode;
...
let passphraseDialogTrigger = null;
let definitionPanelTrigger = null;
...
function show(id) {
  ...
  document.body.classList.toggle('report-mode', id === 'reportScreen');
...
function openPassphraseDialog(mode) { const dialog = $('#passphraseDialog'); if (!dialog) return; passphraseDialogTrigger = document.activeElement instanceof HTMLElement ? document.activeElement : null; dialog.dataset.mode = mode; dialog.hidden = false; $('#passphraseTitle').textContent = mode === 'protect' ? 'Protéger mes observations' : 'Déverrouiller mes observations'; $('#passphraseHelp').textContent = mode === 'protect' ? 'Choisis une phrase de 8 caractères minimum. Elle ne sera jamais enregistrée.' : 'Entre ta phrase secrète. Elle n’est jamais envoyée ni conservée.'; $('#passphraseConfirmField').hidden = mode !== 'protect'; $('#passphrase').value = ''; $('#passphraseConfirm').value = ''; $('#passphrase').focus(); }
function closePassphraseDialog() { const dialog = $('#passphraseDialog'); if (dialog) dialog.hidden = true; const trigger = passphraseDialogTrigger; passphraseDialogTrigger = null; if (trigger && typeof trigger.focus === 'function') trigger.focus(); }
function handlePassphraseDialogKeydown(event) { const dialog = $('#passphraseDialog'); if (!dialog || dialog.hidden) return; if (event.key === 'Escape') { event.preventDefault(); closePassphraseDialog(); return; } if (event.key !== 'Tab') return; const focusable = [...dialog.querySelectorAll('input, button')].filter(element => !element.disabled && !element.hidden); if (!focusable.length) return; const first = focusable[0]; const last = focusable[focusable.length - 1]; if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus(); } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus(); } }
document.addEventListener('keydown', handlePassphraseDialogKeydown);
...
document.addEventListener('click', event => {
  ...
  const part = event.target.closest('#wheelPanel .interactive-sector, #wheelPanel .extra-emotion, #wheelPanel .blend-emotion');
  if (!part) return;
  draft.emotion = null;
...
function renderExercise() {
  ...
  if (evidence) {
    ...
    const sourceLinks = studies.map(study => `<a href="${escapeHtml(study.pubmed_url || study.source_url || '#')}" target="_blank" rel="noopener noreferrer">${escapeHtml(study.title || study.authors?.slice?.(0, 2)?.join(', ') || 'Référence scientifique')}</a>`).join(' · ');
...
async function shareReportText(shareText, status) {
  try {
    if (navigator.share) {
      await navigator.share({ title: 'Résumé de séance — Émotions', text: shareText });
      status.textContent = 'Résumé prêt à être envoyé.';
    } else if (navigator.clipboard?.writeText) {
      await navigator.clipboard.writeText(shareText);
      status.textContent = 'Résumé copié. Tu peux maintenant le coller dans ton message.';
    } else {
      const link = document.createElement('a');
      link.href = URL.createObjectURL(new Blob([shareText], { type: 'text/plain;charset=utf-8' }));
      link.download = 'resume-seance-emotions.txt';
      link.click();
      URL.revokeObjectURL(link.href);
      status.textContent = 'Résumé téléchargé en texte.';
    }
  } catch (error) {
    if (error.name !== 'AbortError') status.textContent = 'Le partage n’est pas disponible dans ce navigateur.';
  }
}
function renderInlineReport(observationCards, shareText, count) {
  const screen = $('#reportScreen');
  if (!screen) return;
  $('#reportInlineBody').innerHTML = `<header class="report-cover"><div class="report-brand">ÉMOTIONS</div><h2>Résumé de séance</h2><p>Une trace claire de ce que tu as observé, ressenti et essayé.</p><div class="report-summary">Nombre d’observations : ${count}</div></header>${observationCards || '<p>Aucune observation enregistrée.</p>'}`;
  $('#reportInlineBack').onclick = () => show('history');
  $('#reportInlinePrint').onclick = () => window.print();
  $('#reportInlineShare').onclick = () => shareReportText(shareText, $('#reportInlineStatus'));
  show('reportScreen');
}
function report() {
  ...
  const plain = value => value || 'Non renseigné';
...
function renderExerciseLibrary() {
  ...
  list.innerHTML = scientific.exercises.map(exercise => {
    ...
    const professionalOnly = exercise.professional_only === true;
...
function renderPrivacyStatus() {
  ...
  if (!status) return;
  if (LOCAL_ONLY_MODE) status.innerHTML = `<b>Observations locales</b><p>Cette bêta fonctionne sans compte et sans phrase secrète. Tes nouvelles observations restent dans ce navigateur.</p>${legacySecureStorageDetected ? '<p class="privacy-recovery-note"><b>Ancienne protection détectée :</b> un test précédent a laissé une donnée chiffrée séparée. Elle n’est pas utilisée par le parcours actuel. Le bouton rouge permet de nettoyer le stockage local si tu le souhaites.</p>' : ''}`;
  else if (stateLocked) status.innerHTML = `<b>Observations protégées</b><p>Une phrase secrète protège déjà les observations de cet appareil. Déverrouille-les avec cette phrase pour les relire ou modifier la protection.</p><p class="privacy-recovery-note"><b>Phrase oubliée ?</b> Elle ne peut pas être récupérée par l’application. La seule réinitialisation possible efface les observations chiffrées avec le bouton rouge ci-dessous.</p><button id="privacyUnlock" class="secondary-action">Déverrouiller mes observations</button>`;
...
$('#privacyDelete').onclick = () => { if (!confirm('Supprimer les observations et le brouillon de cet appareil ?')) return; state.entries = []; localStorage.removeItem(KEY); localStorage.removeItem(SECURE_KEY); localStorage.removeItem(DRAFT_KEY); localStorage.removeItem(SECURE_DRAFT_KEY); secureMode = false; stateLocked = false; legacySecureStorageDetected = false; sessionPassphrase = null; clearDraft(); renderPrivacyStatus(); renderHistory(); };
if ($('#passphraseDialog')) $('#passphraseDialog').onsubmit = event => { event.preventDefault(); submitPassphrase(); };
if ($('#passphraseCancel')) $('#passphraseCancel').onclick = closePassphraseDialog;
...
$('#definitionButton').onclick = event => { definitionPanelTrigger = event.currentTarget instanceof HTMLElement ? event.currentTarget : null; $('#definitionPanel').classList.add('open'); $('#tools').classList.add('show-definition'); window.scrollTo(0, 0); const definitionHeading = $('#definitionPanel h2'); if (definitionHeading) { definitionHeading.setAttribute('tabindex', '-1'); definitionHeading.focus({ preventScroll: true }); } };
